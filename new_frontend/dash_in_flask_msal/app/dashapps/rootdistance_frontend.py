...
def add_dash(server):
    ...
    def perform_volcano_query(
        ...
        # response = requests.post(base_url_api + "/metadataresource/", json=metadata_json_output)
        ...
        #call api
        response = requests.post(base_url_api + "/rootdistanceresource/", json=root_distance_json_output)
        total_panda = pd.read_json(response.json(), orient="records")
